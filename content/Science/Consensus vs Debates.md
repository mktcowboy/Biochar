---
tags: [science, debates]
aliases: [Uncertainties, Open questions]
---

# Consensus vs Debates

## Broad consensus

- Well-made biochar (adequate T / low H/C_org) is **much more persistent** than raw biomass; fused aromatics are the mechanistic basis.
- **Sustainable feedstock** (wastes/residues; no deforestation) is required for net climate benefit.
- Biochar can **raise yields** especially in acid/degraded/tropical soils, but responses are **context-specific**.
- **N₂O reductions** are real in many systems in year 1; magnitude and multi-year persistence less certain.
- **MRV** can focus on mass × C fraction × permanence factor at the kiln/gate, with application verification—cheaper than full soil flux monitoring ([[Biochar in Climate Change Mitigation|reviews biochar climate mitigation]] · [DOI](https://doi.org/10.1038/s41561-021-00852-8)).
- Quality control for **PAHs/heavy metals** and process emissions (kiln CH₄) matters for safe deployment.

## Open debates / uncertainties

1. **Long-term decay model form:** Two-pool fits from ≤few-year incubations may mis-estimate millennial recalcitrant pools; newer proxies (HyPy, conductivity, Ro) and policy commentaries argue for refined permanence metrics.
2. **CH₄ in paddies:** Direction and magnitude still study-dependent.
3. **Priming ceiling:** Decade-scale negative priming is promising but long-term saturation and organic-soil risks remain.
4. **Energy vs CDR trade-off:** Relative benefit depends on **what energy is displaced** (coal vs renewables); as grids decarbonize, soil biochar becomes relatively more attractive ([Lehmann 2021](https://doi.org/10.1038/s41561-021-00852-8)).
5. **Economic vs technical potential:** Technical/sustainable multi-Gt ≠ near-term deployable; IAM/economic potentials often **≪1 Gt by 2050**.
6. **Non-soil uses** (concrete, asphalt, animal feed, materials): permanence pathways differ; soil-amendment science does not automatically transfer.
7. **Climate feedback on permanence:** Warmer soils accelerate decay ([[Biochar Soil GHG Inventory Model|models soil GHG from biochar]] · [DOI](https://doi.org/10.1021/acs.est.1c02425) Q10 framework)—future warming reduces F_perm relative to today’s tables.

## Primary links

- [Lehmann et al. 2021](https://doi.org/10.1038/s41561-021-00852-8)
- [Woolf et al. 2021](https://doi.org/10.1021/acs.est.1c02425)
- [Schmidt et al. 2025](https://doi.org/10.1111/gcbb.70092)
- [Weng & Cowie 2025](https://doi.org/10.1038/s43247-025-02228-x)
- [State of CDR / Fuss 2026](https://doi.org/10.17605/OSF.IO/FBZ4X) · [[CDR Costs and Potentials|CDR costs and potentials]]

## Related notes

- [[Permanence]] · [[Permanence Debates]]
- [[Permanence Science for Credits]]
- [[CDR Potential and IPCC Accounting]]
- [[GHG Fluxes]]
- [[Risks and Controversies]]
- [[Science Hub]]
