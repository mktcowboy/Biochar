---
tags: [production, standards, testing, laboratory]
aliases: [Biochar testing, EBC testing, IBI testing, S668 testing]
updated: 2026-09-16
---

# Testing EBC IBI S668

Chapter on **how biochar is tested and certified**: European Biochar Certificate (EBC), IBI Biochar Standards, and ANSI/ASABE/USBI S668 test methods. Clarifies pass/fail product standards vs methods-only standards vs carbon methodologies. Report context: **2026-09-16**.

**Related notes:** [[Contaminants and Quality]] · [[Product Standards EBC IBI]] · [[ANSI ASABE USBI S668]] · [[PAHs]] · [[Heavy Metals and Ash]] · [[PFAS and Emerging Contaminants]] · [[H-Corg and O-Corg]] · [[Properties]] · [[Key Sources]]

---

## 1. Three layers

| Layer | Instrument | Sets pass/fail limits? | Vault home |
|---|---|---|---|
| Product certification | **EBC** / **IBI** (and WBC-related EBC framing) | **Yes** — H/C_org, PAH, metals, use classes | [[Product Standards EBC IBI]] |
| Test methods | **ANSI/ASABE/USBI S668** (Nov 2025) | **No** — recommended methods only | [[ANSI ASABE USBI S668]] |
| Carbon crediting | Verra / Puro / Isometric / CSI | Eligibility + MRV (may require EBC/IBI) | [[MRV Stack]] · methodology notes |

CSI Global Biochar C-Sink requires **EBC- or WBC-certified** biochar ([CSI docs](https://www.carbon-standards.com/docs/transfer/4000039EN.pdf) · [[CSI Global Biochar C-Sink]]).

---

## 2. EBC (v10.5E, 14 Aug 2025)

Primary PDF: [EBC Guidelines 10.5E](https://www.european-biochar.org/media/doc/7/ebc_en_10_5_red.pdf) · home [european-biochar.org](https://www.european-biochar.org/).

**Classes:** FeedPlus, AgroOrganic, Agro, Urban, Materials, Basic (each with purpose-specific limits).

**Minimum analytical package ideas:**

- Elemental: C_tot, C_org, H, N, O, S, ash → **H/C_org** (and O/C_org)
- Physical: moisture, bulk density, WHC, pH, salt/EC, solid EC
- Nutrients: N, P, K, Mg, Ca, Fe
- Contaminants: heavy metals; PAH (Σ16 + Σ8); PCB/PCDD/F when triggered
- FeedPlus extras: dioxin/PCB packages, F, crude ash, etc. (§10)

**Batch & sampling culture:** register batches in Biochar Tool; endorsed sampler; endorsed lab; retention samples; T within ±20%; feedstock blend within ±20% ([§5–6](https://www.european-biochar.org/media/doc/7/ebc_en_10_5_red.pdf)).

Key numeric gates duplicated in contaminant chapters: [[PAHs]], [[Heavy Metals and Ash]], [[H-Corg and O-Corg]].

---

## 3. IBI Biochar Standards

Portal: [biochar-international.org/biochar-standards](https://biochar-international.org/biochar-standards/) · org home [biochar-international.org](https://biochar-international.org/).

IBI standards define biochar material criteria (including H/C_org ≤ 0.7 classically, O/C thresholds, and contaminant declaration/limits by grade) used widely in North American commerce and lab report interpretation. ABI’s interpreting-lab-reports guidance points practitioners to IBI method packages ([ABI lab report PDF](https://biochar.org/wp-content/uploads/2026/06/Biochar-Guidelines-Lab-Report-2026.pdf) · [[US Biochar Initiative]]). Exact grade tables should be read from the current IBI document before diligence—vault does not invent versioned limit tables beyond EBC full-text extracts and the ≤0.7 / ≤0.4 ratios already in [[Properties]].

---

## 4. ANSI/ASABE/USBI S668 (NOV2025ED)

Source note: [[ANSI ASABE USBI S668]] · [PDF](https://biochar.org/wp-content/uploads/2026/05/ANSI-S668.pdf) · [doc page](https://biochar.org/document/ansi-s668/).

**Scope highlights:**

| Topic | S668 pointer |
|---|---|
| Permanence metrics | §10 — H:C_org, O:C_org formulas; Ro per **ISO 7404-5** |
| Organic C | §9 — C_org = total C − inorganic C |
| Physical | §11 — bulk density ISO 17828; PSD sieves; BET ISO 9277 guidance |
| PAHs / PCBs / PCDD/F | §12.1 — EPA extraction/analysis methods |
| **PFAS** | §12.1.4 — **EPA 1633A**; no pass/fail |
| Trace metals | §12.2 — EPA methods for U.S. use |
| Ag parameters | Nutrients, pH, liming, salinity |

Front matter: ASABE standards are informational/advisory/voluntary. S668 **does not replace** NRCS CPS 336 or registry MRV.

---

## 5. How to choose a stack

| Goal | Practical stack |
|---|---|
| Sell soil amendment in EU / EBC markets | EBC class certification + endorsed lab |
| Sell in U.S. with transparent methods | IBI-aligned report + S668/EPA methods; consider EBC if buyer/CDR requires |
| Issue carbon credits | Follow methodology; many paths expect EBC/IBI + production MRV ([[Permanence Science for Credits]]) |
| Research / R&D only | S668 methods + publish FC, H/C_org, PAH, metals |

---

## 6. Practice takeaway

1. Never treat a **methods standard** (S668) as a **certificate**.
2. Match **class** to end use before comparing PAH/metal numbers.
3. For PFAS, report method + jurisdiction—there is no universal biochar PFAS limit in S668.

---

## Primary links

- [EBC 10.5E PDF](https://www.european-biochar.org/media/doc/7/ebc_en_10_5_red.pdf) · [EBC home](https://www.european-biochar.org/)
- [IBI Biochar Standards](https://biochar-international.org/biochar-standards/)
- [ANSI S668 PDF](https://biochar.org/wp-content/uploads/2026/05/ANSI-S668.pdf) · [[ANSI ASABE USBI S668]]
- [ABI Interpreting Lab Reports](https://biochar.org/wp-content/uploads/2026/06/Biochar-Guidelines-Lab-Report-2026.pdf)
- [Bilias et al. 2024](https://doi.org/10.1515/pac-2021-0106) · [[Bilias et al. 2024]]
- [CSI C-Sink PDF](https://www.carbon-standards.com/docs/transfer/4000039EN.pdf)

## Related notes

- [[Contaminants and Quality]] · [[Product Standards EBC IBI]] · [[PAHs]] · [[Heavy Metals and Ash]] · [[PFAS and Emerging Contaminants]] · [[Production Hub]]
