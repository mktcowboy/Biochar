---
tags: [applications, economics, agriculture, costs]
aliases: [Biochar farm economics, Cost benefit biochar]
updated: 2026-09-16
---

# Agronomic Economics

Chapter on **farm-level cost and benefit** of soil-applied biochar and blends. Sourced figures only. NPV is sensitive to **delivered $/ton**, **application rate**, **yield response**, **crop price**, and whether **carbon finance** or **NRCS cost-share** is stacked. Do not treat any single trial as a universal ROI.

Report context: **2026-09-16**.

**Sibling notes:** [[Cost Components]] · [[Fertilizer Displacement]] · [[Field Rates by System]] · [[Crop Yield Effects]] · [[Real World Soil Projects]] · [[Matrix Pathways for Credits]] · [[NRCS CPS 336 and US Ag]] · [[Market Volumes and Pricing]].

---

## 1. Headline insight from peer-reviewed economics

Galinato, Yoder & Granatstein (2011), *Energy Policy* — “The economic value of biochar in crop production and carbon sequestration” ([DOI](https://doi.org/10.1016/j.enpol.2011.07.035); ScienceDirect: https://www.sciencedirect.com/science/article/abs/pii/S0301421511005672):

- Evaluates biochar for **Washington winter wheat** with and without carbon payments.
- Finds profitability generally requires **low biochar prices** and/or a **carbon offset market**; agronomic value alone is often insufficient at then-prevailing prices and the high application rates modeled.
- Use as a caution against assuming fertilizer-like payback from bulk raw char in temperate systems—consistent with weak temperate mean yields in [[Jeffery et al. 2017]] ([DOI](https://doi.org/10.1088/1748-9326/aa67bd)).

Exact break-even tables are in the paper; quote numbers from the PDF/HTML when doing diligence rather than secondary paraphrases. This vault does **not** invent those table cells without full-text check.

---

## 2. Multi-year vineyard field economics (California)

### Oasis Vineyard Trial (Monterey County) — primary PDF

Source: https://pacificbiochar.com/wp-content/uploads/Oasis-Vineyard-Trial_-5th-harvest_-Final-Draft_12.12.2023.pdf · [[Oasis Vineyard Trial 2023]]

| Item | Value (as reported) |
|---|---|
| Biochar delivered price (2016, wet) | **$240 / ton** |
| Compost delivered price (2016, wet) | **$40 / ton** |
| Grape price used (Oct 2023) | ~**$1,500 / ton** |
| Rates | Biochar **10 t/ac**; compost **15 t/ac**; combo both |
| Placement | GPS rip + winged plow bowl ~2×2.5 ft to ~30" |
| Five-season cumulative yield | Combo **41.7 t/ac**; compost 40.1; biochar 38.2; control **31.7** |
| Average yield Δ vs control | Combo **+36%**; compost **+29%**; biochar **+25%** |
| Extra revenue by 5th harvest vs control | Biochar **$9,752/ac**; compost **$12,569/ac**; combo **$14,937/ac** |
| Rough profit after amendment cost | Biochar **$7,352/ac**; compost **$11,969/ac**; combo **$11,937/ac** |
| Payback | Positive ROI by **second harvest** for all treatments |
| Illustrative extrapolation (biochar alone) | Authors’ scenario: if next 5 years at half observed gain → ~**$12,228/ac** over 10 yr (~5× biochar investment); 20-harvest stretch ~**$21,980/ac** |

**Caveats stated in report:** revenue highly dependent on grape price; same irrigation/fertilizer cost across treatments (so water/fertilizer savings **not** monetized — see [[Fertilizer Displacement]]); woody high-T char persistence cited via [[Woolf et al. 2021]]. Compost alone out-earned biochar alone; combination did best on revenue.

### Dominus Estate

Public case study emphasizes SOM targeting and earthworks at **25 t/ac**; it does **not** publish a full ROI table comparable to Oasis—do not invent one ([case study](https://pacificbiochar.com/case-study-biochar-application-with-dominus-estate-in-napa-valley-after-2-years-fallow/)).

---

## 3. US cost-share (NRCS CPS 336)

- Practice is **financial assistance** (EQIP / related), **not** a carbon credit — see [[Double Counting and Credit Ownership]]. FAQ does not treat CPS 336 as a credit methodology ([FAQ PDF](https://nrcs-prod.azureedge.us/sites/default/files/2026-04/soil-carbon-amendment-ac-336-faq.pdf) · [[NRCS CPS 336 FAQ 2026]]).
- Scenario payment example (Farmland Info guide): FY2023 **60/40 biochar/compost** constructed ~**$160 / cubic yard** covering average purchase + logistics + application components; **varies by state**; not matched to actual invoices ([PDF](https://farmlandinfo.org/wp-content/uploads/sites/2/2023/12/biochar_funding_for_US_producers_quick_guide_.pdf)). Line-item framing: [[Cost Components]].
- Ranking: 336 alone often ranks low; stack with other CSAF soil-health practices ([FAQ](https://nrcs-prod.azureedge.us/sites/default/files/2026-04/soil-carbon-amendment-ac-336-faq.pdf)).
- Lifespan **1 year**, up to **5** contracts ([FAQ](https://nrcs-prod.azureedge.us/sites/default/files/2026-04/soil-carbon-amendment-ac-336-faq.pdf)).
- Funding guides: ABI 2026 https://biochar.org/wp-content/uploads/2026/06/Biochar-Guidelines-Funding-2026-final.pdf · [[NRCS CPS 336 and US Ag]].

---

## 4. Supplier / market price signals (use carefully)

| Signal | Figure | URL | Caveat |
|---|---|---|---|
| Oasis 2016 delivered biochar | $240/ton wet | Oasis PDF above | One CA forest-residue supply chain, dated |
| Oasis 2016 delivered compost | $40/ton wet | Oasis PDF | Local greenwaste compost |
| Farmland Info FY2023 scenario | ~$160/yd³ average cost components | https://farmlandinfo.org/wp-content/uploads/sites/2/2023/12/biochar_funding_for_US_producers_quick_guide_.pdf | Payment construction, not invoice |
| Wakefield bulk packaging | 2 yd³ supersacks; 2–3 sacks/ac typical farm cite | https://wakefieldbiochar.com/biochar-for-landscapers/ | No public $/ton on that page |
| CDR credit prices | See market notes | [[Market Volumes and Pricing]] · https://www.cdr.fyi/ | Credit ≠ farmgate amendment price |
| State of CDR / science cost bands | ~$70–360/tCO₂ (removal cost literature) | https://doi.org/10.17605/OSF.IO/FBZ4X · [[Fuss & Lunstrum 2026]] · [[Quantitative Benchmarks]] | System CDR cost, not retail bag price |

---

## 5. Brazil pasture CBA

Latawiec et al. (2019) include an environmental and **cost-benefit analysis** for biochar on degraded Brazilian pastures ([DOI](https://doi.org/10.1038/s41598-019-47647-x); PDF https://www.iis-rio.org/wp-content/uploads/2019/10/Latawiec_et_al-2019-Scientific_Reports.pdf). Use paper tables for local currency figures rather than converting without citation.

---

## 6. Pathways that improve agronomic ROI (evidence-based)

| Lever | Why it can help | URL / note |
|---|---|---|
| **Blends / COMBI** | Oasis combo highest revenue; Kammann co-composted >> raw | Oasis PDF · https://doi.org/10.1038/srep11080 · [[Biochar Compost and Manure Blends]] |
| **Low-dose enriched fertilizers** | ≪2 t ha⁻¹ possible when nutrient-loaded | https://doi.org/10.1002/ldr.2761 · [[Melo et al. 2022]] https://doi.org/10.1007/s11104-021-05276-2 |
| **Banding / root-zone** | Cuts total tonnes vs whole-field broadcast | [[Incorporation Topdress Banding]] · Oasis row placement |
| **Target acid / coarse / tropical soils** | Stronger mean yield metas | https://doi.org/10.1088/1748-9326/aa67bd · [[Crop Yield Effects]] |
| **Fertilizer co-application** | Ye ~+15% vs fertilizer alone | https://www.css.cornell.edu/faculty/lehmann/publ/SoilUseManage%2036%2C%202-18%202020%20Ye.pdf · [[Fertilizer Displacement]] |
| **CPS 336 +/or CDR stack** | Partial cost offset — with ownership hygiene | [[Matrix Pathways for Credits]] · [[Double Counting and Credit Ownership]] |
| **Facility co-composting** | Possible fuel/labor savings narrative (ABI draft) | https://biochar.org/wp-content/uploads/2026/06/Biochar-Guidelines-Compost-2026-Draft-V1.pdf |

Do **not** bank multi-year soil N₂O co-benefits in NPV unless measured—Woolf optional inventory term only at **>10 Mg C ha⁻¹** year 1 ([[Nitrous Oxide Fluxes]], [DOI](https://doi.org/10.1021/acs.est.1c02425)).

---

## 7. Decision framework (sourced logic)

1. **Estimate agronomic delta** from climate/soil analogues ([[Crop Yield Effects]] — tropics/acid/coarse stronger).
2. **Price the amendment** delivered + incorporation ([[Cost Components]]; Oasis and NRCS scenario docs as templates).
3. **Add policy stack:** CPS 336 cost-share and/or voluntary CDR offtake—with legal separation of claims.
4. **Stress-test** crop price and zero-yield-response cases (Galinato caution for temperate wheat systems).
5. Prefer **blends / enriched fertilizers / banding** when aiming to cut rate and cost ([[Biochar Compost and Manure Blends]], Nepal low-dose evidence https://doi.org/10.1002/ldr.2761 · [[Schmidt et al. 2017]]).
6. Run a local strip trial before scaling ([[On-farm Trial Design]]).

---


---

## 8. Worked template (Oasis inputs only)

Illustrative structure using **only published Oasis figures**—not a universal calculator:

1. Amendment cost/ac ≈ (10 t biochar × $240) + (optional 15 t compost × $40) = **$2,400** biochar-only or **$3,000** combo materials at 2016 delivered wet prices ([Oasis PDF](https://pacificbiochar.com/wp-content/uploads/Oasis-Vineyard-Trial_-5th-harvest_-Final-Draft_12.12.2023.pdf)).
2. Cumulative extra revenue by harvest 5 at ~$1,500/t grapes: biochar **$9,752/ac**, combo **$14,937/ac** (same PDF).
3. Authors’ rough profit after amendment cost: biochar **$7,352/ac**, combo **$11,937/ac** (same PDF).
4. Stress tests the report itself flags: grape price; no water/fertilizer savings counted; persistence assumed via Woolf for high-T wood char.

For NRCS-offset scenarios, subtract expected CPS 336 payment (state schedule) from net cost—using Farmland Info’s ~**$160/yd³** only as a **scenario construction** example, not your invoice ([PDF](https://farmlandinfo.org/wp-content/uploads/sites/2/2023/12/biochar_funding_for_US_producers_quick_guide_.pdf)).

---

## 9. When temperate systems likely need a stack

| Evidence | Implication | URL |
|---|---|---|
| Jeffery 2017 temperate mean little/no yield benefit | Agronomy-alone ROI weak for raw char | https://doi.org/10.1088/1748-9326/aa67bd |
| Galinato 2011 WA wheat | Needs low biochar price and/or carbon payments | https://doi.org/10.1016/j.enpol.2011.07.035 |
| Oasis sandy low-OM vineyard | Strong multi-year ROI **with** compost treatments on a high-value perennial | Oasis PDF |
| Melo / Schmidt low-dose BBFs | Cut tonnes → cut $/ac if enrichment works | https://doi.org/10.1007/s11104-021-05276-2 · https://doi.org/10.1002/ldr.2761 |

---

## 10. Linking to credit prices without double counting

CDR market prices ([[Market Volumes and Pricing]], https://www.cdr.fyi/) can close Galinato-style gaps **only if**:

1. The tonne is eligible under a methodology soil pathway ([[Matrix Pathways for Credits]]),
2. Ownership is clear if CPS 336 also paid ([[Double Counting and Credit Ownership]]),
3. You do not count flux co-benefits twice ([[GHG Fluxes]]).

Literature removal **cost** bands (~$70–360/tCO₂ — [[Fuss & Lunstrum 2026]] · [DOI](https://doi.org/10.17605/OSF.IO/FBZ4X)) are not credit bid/ask.

## Related notes

- [[Cost Components]] · [[Fertilizer Displacement]] · [[Soil and Agriculture]] · [[Application Rates and Methods]]
- [[Real World Soil Projects]] · [[Market Volumes and Pricing]] · [[Buyers and Offtakes]] · [[Applications Hub]]
