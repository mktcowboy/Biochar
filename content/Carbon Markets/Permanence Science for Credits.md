---
tags: [carbon-markets, permanence]
aliases: [Credit permanence, H/C_org for credits]
---

# Permanence Science for Credits

Same science, different program interpretations. Do not mix thresholds across registries.

| Source | Threshold / use | Horizon |
|---|---|---|
| **[IPCC 2019](https://www.ipcc-nggip.iges.or.jp/public/2019rf/pdf/4_Volume4/19R_V4_Ch02_Ap4_Biochar.pdf) · [[IPCC Biochar Soil Carbon Method|estimates soil C stock change from biochar]]** | Pyrolysis **temperature class** for F_perm; H/C_org & O/C as future options | 100 years |
| **NRCS CPS 336** | H:C_org **max 0.7** (practice eligibility—not a credit) | Soil amendment |
| **Isometric soil** | H/C_org **< 0.5**, O/C_org **< 0.2**; F_durable from Woolf-style (200y) or Ro (1000y) | 200 or 1000 years |
| **CSI Global Biochar C-Sink 3.3** · [[CSI GPC vs SPC Permanence Classes]] | H/C_org **≤ 0.40** for any GPC; **> 0.40** = SPC only; Ro≥3.8% or HyPy≥90% → **90% GPC** | GPC >1000y; SPC ≤1000y |
| **Verra VM0044** | H/C_org → Table 3 permanence factors; high ratios excluded | 100 years |
| **Puro Edition 2025** | CORC200+ (“several centuries”); revised decay; Ro encouraged but **does not yet quantify** | Several centuries |
| **EBC/WBC** | Product quality classes feeding CSI / referenced by others | Quality + C-sink inputs |

## Two camps (active divergence)

- **Decomposition-model camp:** IPCC appendix, [[Biochar Soil GHG Inventory Model|models soil GHG from biochar]], Puro CORC200+, Isometric 200y
- **Inertinite / Ro camp:** [[Inertinite Permanence Benchmark|inertinite permanence benchmark]]; Isometric 1000y; CSI upper class

Puro explicitly chose **centuries not millennia** pending more integrated science ([Puro permanence post, 2025-07-03](https://puro.earth/insights/post/biochar-permanence/)).

See also science-side [[Permanence]] hub ([[H-Corg and O-Corg]], [[Permanence Factors Fperm|inventory permanence factors]], [[Reflectance Ro and HyPy]], [[Soil Temperature and Environment]], [[Permanence Debates]]) and [[Consensus vs Debates]]. Do not duplicate methodology thresholds into those science notes.

## Primary links

- [IPCC 2019 App. 4](https://www.ipcc-nggip.iges.or.jp/public/2019rf/pdf/4_Volume4/19R_V4_Ch02_Ap4_Biochar.pdf)
- [Woolf et al. 2021](https://doi.org/10.1021/acs.est.1c02425)
- [Schmidt et al. 2025](https://doi.org/10.1111/gcbb.70092)
- [Sanei et al. 2024](https://doi.org/10.1016/j.coal.2023.104409)
- [Puro permanence post](https://puro.earth/insights/post/biochar-permanence/)
- [Verra VM0044](https://verra.org/methodologies/vm0044-biochar-utilization-in-soil-and-non-soil-applications-v1-2/)
- [Isometric Biochar v1.3](https://registry.isometric.com/protocol/biochar/1.3)
- [CSI C-Sink 3.3](https://www.carbon-standards.com/docs/transfer/4000039EN.pdf)

## Related notes

- [[Verra VM0044]] · [[Puro CORC200+]] · [[Isometric Biochar Protocol]] · [[CSI Global Biochar C-Sink]] · [[CSI GPC vs SPC Permanence Classes]]
- [[MRV Stack]]
- [[Risks and Controversies]]
- [[Carbon Markets Hub]]
